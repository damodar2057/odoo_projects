...
class EmployeeDashboard(http.Controller):
    ...
    @http.route('/absent-history', auth='user', website=True)
    def absent_page(self, **kw):
        user = request.env.user
        current_employee = request.env['hr.employee'].sudo().search([('user_id','=',user.id)],limit=1)

        show=True
        # Get the employee's attendance records for the last 30 days
        employee_attendance_ref = request.env['hr.attendance'].sudo().search([
            ('employee_id', '=', current_employee.id),
            ('check_in', '>=', (datetime.now() - timedelta(days=30)).replace(hour=0, minute=0, second=0, microsecond=0))
        ])
        # Extract unique check_in dates from the attendance records
        employee_attendance_dates = {attendance.check_in.date() for attendance in employee_attendance_ref}

        print("***********************************************")
        print(employee_attendance_dates)
        print("***********************************************")
        
        # Get today's date
        today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)

        # Generate a list of datetime objects for the past 30 days excluding Saturdays
        past_30_days = []
        for i in range(30):
            dt = today - timedelta(days=i)
            if dt.weekday() != 5:  # 5 corresponds to Saturday
                past_30_days.append(dt.date())

        # Remove attendance dates from past 30 days
        past_30_days_not_attended_dates = [dt for dt in past_30_days if dt not in employee_attendance_dates]
        # Sort the filtered list of datetime objects in descending order
        past_30_days_filtered_sorted = sorted(past_30_days_not_attended_dates, reverse=True)

  
        return request.render("employee_dashboard.not_attended_page",{'show':show,'not_attended_dates':past_30_days_not_attended_dates})
    ...
    def attendance_page(self, **kw):
        ...
        is_mobile = 'mobile' in parse_accept_header(user_agent_string) or 'iPhone' in parse_accept_header(user_agent_string)
    ...
    def dashboard_page(self, **kw):
        ...
        # this below is for the notification of daily reporting missed and attendance missed
        # know the current user id
        current_user = request.env.user
        emp_ref = request.env['hr.employee'].sudo().search([('user_id','=',current_user.id)],limit=1)
        # Fetch all the notification for the current user
        notification_ref = request.env['notification.model'].search([('emp_id','=',emp_ref.id),('message_seen_status','=',False)])
        notification_count = len(notification_ref)
        print("*********************")
        ...
        return request.render("employee_dashboard.dashboard_page",
                              {'total_present_days':total_present_days,
                               ...
                               'show':show,
                               'dept':dept_details,
                               'notification_list':notification_ref,
                               'notification_count':notification_count,
                               })
        
    @http.route('/removeCheckedNotification/<int:id>', auth='user',method=['GET'])
    def remove_checked_notification(self,id, **kw):
        user = request.env.user
        not_ref = request.env['notification.model'].search([('id','=',id)],limit=1)
        not_ref.write({
                'message_seen_status':True
            })
        return None
