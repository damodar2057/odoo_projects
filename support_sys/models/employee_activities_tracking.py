...
class EmployeeTaskAssignment(models.Model):
    ...
    task_initiated_time = fields.Datetime("Task Initiated Time")
    task_completed_time = fields.Datetime("Task Completed Time")
...
class EmployeeDailyReport(models.Model):
    ...
    def check_daily_reporting_status(self):
        print("*#*#*#*#*#*#*#*#*#*#")
        print("I am from daily reporting status")
        print("*#*#*#*#*#*#*#*#*#*#")
        today = datetime.now()
        emp_ref = self.env['hr.employee'].search([])
        for emp in emp_ref:
            emp_daily_report_data = self.env['employee.daily.work.report'].search([('employee_id','=',emp.id),('work_date','=',datetime.now().date())])
            if (not emp_daily_report_data):
                print(emp.user_id)
                self.env['notification.model'].create({
                    'message_seen_status':False,
                    'emp_id':emp.id,
                    'message':"You have missed uploading daily report!!!",
                    'sent_to':emp.user_id.id,
                    'sent_at':today,
                })
                
                print(f"{emp.name} has not posted daily report ")
            else:
                print(f"{emp.name} has  posted daily report ")
